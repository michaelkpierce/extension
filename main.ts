namespace PCA9685 {
    ...
    */
    ...
    export function setCRServoPosition(servoNum: ServoNum = 1, speed: number, chipAddress: number = 0x40): void {
        ...
        debug(`Spread ${spread2}`)
        servo2.position = speed
        speed = Math.abs(speed)
        const calcOffset2: number = ((speed * spread2) / 100)
        debug(`Offset ${calcOffset2}`)
        debug(`min ${offsetStart}`)
        debug(`mid ${offsetMid}`)
        debug(`max ${offsetEnd}`)
        const pwm3 = isReverse ? offsetMid - calcOffset2 : offsetMid + calcOffset2
        debug(`pwm ${pwm3}`)
        return setPinPulseRange(servo2.pinNumber, 0, pwm3, chipAddress)
    }
    ...
    export function chipAddress(hexAddress: string): number {
        ...
        for (let k = 0; k < l; k++) {
            const char = hexAddress.charAt(k)
